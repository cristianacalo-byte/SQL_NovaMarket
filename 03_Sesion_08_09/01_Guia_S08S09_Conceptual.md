...
|---|---|
| ¿Cuántas filas retorna GROUP BY? ¿Por qué? | Retorna **6 filas**. Porque agrupa los 500 registros originales en 6 grupos únicos, uno por cada `CiudadID` presente en la tabla. |
...
| Pregunta | Tu respuesta |
|---|---|
| ¿Qué CiudadID tiene Margen_Aproximado negativo? | **CiudadID = 2** (que corresponde a Leticia). |
...
| Pregunta | Tu respuesta |
|---|---|
| ¿Para decidir si cerrar Leticia, cuál usarías: SUM o AVG? | Usaría **SUM**, porque necesitamos saber la pérdida o impacto total acumulado en caja (el dinero total perdido), no solo el promedio por transacción. |
...
| ¿Por qué ahora aparece 'Leticia' y no '6'? | Porque el `INNER JOIN` cruzó la tabla de ventas con la dimensión ciudad (`DimCiudad`), trayendo el campo descriptivo `c.Ciudad` asociado al ID. |
...
| ¿Cuánto es esa pérdida? | **-$55,886.00** |
| ¿Coincide este resultado con el dashboard de Power BI de S4? | **SÍ**, coincide exactamente con los valores y la tendencia observada en Power BI. |
...
SELECT 
    ...
    ROUND(
        SUM(f.Precio_Venta * f.Cantidad * (1 - f.Descuento_Pct) - f.Costo_Unitario * f.Cantidad - f.Costo_Envio) / 
